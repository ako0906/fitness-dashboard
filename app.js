...
return `<svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="none"> <polyline points="${points.join(' ')}" stroke="${color}" stroke-width="1.2" fill="none" stroke-linejoin="round" stroke-linecap="round" vector-effect="non-scaling-stroke" /> </svg>`;
}
...
$(‘wwGrid’).innerHTML = `<div class="ww-cell"> <span class="ww-label">이번주</span> ${rows.map(r =>`<div class="ww-row"><span>${r.label}</span><span>${r.now}</span></div>`).join('')} </div> <div class="ww-cell"> <span class="ww-label">지난주</span> ${rows.map(r => `<div class="ww-row"><span>${r.label}</span><span>${r.prev}</span></div>`).join('')} </div> <div class="ww-cell"> <span class="ww-label">변화</span> ${rows.map(r => `<div class="ww-row"><span></span>${r.delta || ‘<span>—</span>’}</div>`).join('')} </div> `;
}
...
$(‘insights’).innerHTML = items.map(i => `<div class="insight"> <span class="insight-label">${i.label}</span> <span class="insight-value ${i.cls}">${i.value}</span> </div>`).join(’’);
}
